...
func initAPIs(r repos, s services, c controllers, enr *enricher.Enricher, cacheDir string, db *sql.DB) apis {
	...
	binaryAPI := api.NewBinaryOperation(s.binary, r.vm, cacheDir, s.config)
	imgAPI := api.NewImageOperation(s.image, db, cacheDir, s.config)
	kernelAPI := api.NewKernelOperation(s.kernel, r.vm, cacheDir, s.config, db)
	...
	volumeAPI := api.NewVolumeOperation(s.volume, r.volume, r.vm, cacheDir, db)
