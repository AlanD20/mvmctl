...
`EnvSpec` uses a dynamic `Steps` map keyed by Registry keys, decoded via custom `UnmarshalYAML` that checks each YAML key against `Registry`. Unknown keys are silently ignored.

Resolution flow:
1. Read YAML file → `yaml.Unmarshal` → custom `UnmarshalYAML` populates `Steps`
