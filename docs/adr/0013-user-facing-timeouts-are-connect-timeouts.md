...
`mvm ssh --cmd` and `mvm vm exec --timeout` were interpreted as **absolute command-duration limits**. A command such as

```bash
mvm ssh myvm --cmd 'mycustomcommand'
```

was killed after the configured `ssh_timeout_sec` (default 10s) even when the command was actively making progress. The same applied to `mvm vm exec --timeout`.
...
| `mvm vm exec --timeout` | Total budget for agent probe + guest command execution | Vsock agent probe/connect only |
...
- `mvm vm exec myvm --timeout 30 -- 'long-task'` waits up to 30s for the agent, then runs `long-task` until it finishes.
...
- `pkg/api/vm.go`: `input.Timeout` overrides `defaults.vm.vsock_probe_timeout`; `client.Exec` receives `0` for the command timeout.
