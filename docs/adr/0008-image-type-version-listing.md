...
> **Implementation Note:** A new `VersionInfo` class was introduced in `models/version.py` for use by the shared resolver, while `ImageVersion` in `models/image.py` was retained for backward compatibility with image-specific consumers. The wrapper in `core/image/_version_resolver.py` converts between the two.
