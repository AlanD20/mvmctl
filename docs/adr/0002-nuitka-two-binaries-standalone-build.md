...
- **Release mode**: The default build mode uses aggressive optimization (LTO, anti-bloat, `--deployment`) for minimal binary size (~35 MB).
...
- **No PyInstaller by default**: The `pyinstaller` dependency remains in `pyproject.toml` build group. PyInstaller remains available as an alternative build path via `task build-pyinstaller`, but the primary build system is Nuitka. A PyInstaller fallback hook exists at `src/mvmctl/__pyinstaller/hook-mvmctl.py` but is unused in the default build — the project builds with Nuitka, not PyInstaller.
- **Build targets**: `python scripts/build_services.py` builds everything (default). Use `--services` or `--service <name>` to build specific targets.
- **`--python-flag=no_docstrings` and `--python-flag=no_asserts` are SERVICE-only flags**: These flags are in `SERVICE_FLAGS` (applied to `mvm-services`) but NOT in `MAIN_FLAGS` (applied to the `mvm` main CLI binary). Docstrings and assertions are preserved in the main CLI binary for help text rendering and debugging purposes. The service binaries (console relay, nocloud server, provisioner) are not user-facing and can safely strip both.
- **Different `--nofollow-import-to` for `*.tests` by binary**: The `SERVICE_FLAGS` use `--nofollow-import-to=*.tests` (safe because services have no force-included modules with a `.tests` subpackage). The `MAIN_FLAGS` deliberately avoid `--nofollow-import-to=*.tests` — the fnmatch pattern `*.tests` would also match `jinja2.tests`, which IS force-included for the main CLI. Instead, the main binary uses `--nofollow-import-to=pytest` and `--nofollow-import-to=_pytest`.

> **Implementation Notes:** The build script's `_write_build_version()` function bakes `BUILD_VERSION` into `src/mvmctl/_build_version.py` at build time, and cleans it up in a `finally` block via `_clean_build_version()`. The build sets `CCFLAGS=-Os` and `LDFLAGS=-Os` environment variables for service binary optimization (applied in the `_build_service()` helper).
