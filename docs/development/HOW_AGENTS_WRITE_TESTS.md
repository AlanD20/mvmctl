...
5. [Test Writing Rules](#5-test-writing-rules)
6. [What Agents Must NOT Do](#6-what-agents-must-not-do)
7. [Before Submitting, Self-Check](#7-before-submitting-self-check)
...
### 5.1 Available Fixtures

| Fixture | Scope | Creates | Best For |
|---------|-------|---------|----------|
| `mvm_binary` | session | Nothing | All tests — provides the mvm binary path |
...
### 5.4 Module-Scoped Fixtures for Read-Only Tests
...
### 5.5 Destructive Tests Must Be Last
...
### 5.6 Modifying Existing Test Files
...
## 7. Before Submitting, Self-Check
...
[ ] Does the test pass? (run with --system --test <file>)
```
