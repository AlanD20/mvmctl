...
| 94 | **Go maps require synchronization for concurrent access** | Writing to a Go `map` from multiple goroutines without synchronization causes a **runtime panic** (`fatal error: concurrent map writes`). This is by design — the Go blog, FAQ, and runtime all agree. Read-only concurrent access is safe ONLY when no goroutine is writing. Concurrent reads+writers must use `sync.Mutex` (or `sync.RWMutex` for read-heavy). `sync.Map` is specialized for specific patterns (write-once/read-many caches, disjoint key sets) and is NOT a general replacement — the docs explicitly say "Most code should use a plain Go map instead, with separate locking or coordination." Standard pattern: `map[K]V + sync.Mutex`. |
| 95 | **Parallelize HTTP-bound config resolution per config, still sequential between phases** | In `HttpDirVersionResolver.Resolve()`, each resolution phase (http-dir, single-source, firecracker-s3) must be parallelized per-config using goroutines + `sync.WaitGroup` + `sync.Mutex` protecting the result map. BUT phases must remain sequential because later phases check for existence from earlier phases (`if _, exists := result[typeName]; exists { continue }`). Each goroutine collects results into a local map, then merges into the shared result under the mutex after its work finishes. This reduces wall-clock time for image/kernel `ls -r` from O(N sequential HTTP requests) to O(1) parallel HTTP requests per phase. |
| 96 | **ProvisionerType resolved once at startup, not per-call** | Go is correct: `Operation.ProvisionerType` is set once in `NewOperation()` by reading `settings.guestfs_enabled`. Python resolved it from the DB on every `ImagePull` call, which is wasteful. If the setting changes, the user must restart the daemon — this is acceptable and avoids N+1 DB queries. Do NOT replicate Python's per-call resolution pattern. |
| 97 | **No backend fallback in extractDiskImage** | Go does NOT replicate Python's loop-mount fallback when qemu-img fails. If `convertToRaw` fails, the error propagates directly. The Python fallback was a workaround for older qemu-img versions that are no longer relevant. Do NOT add fallback logic — fail fast and let the user fix the root cause. |
...
| 102 | **Cancelled-context-in-cleanup trap** | When a goroutine triggers cleanup via `<-ctx.Done()`, the context is already cancelled — passing it to cleanup operations (TAP removal, lease release, filesystem operations) causes them to abort silently with `context.Canceled`. **Always use `context.Background()` for cleanup in signal-handling goroutines.** The synchronous error path (cleanup after `execute()` returns) can use `ctx` because it's still alive at that point. This is the **cancelled-context-in-cleanup trap**: a goroutine launched to handle cancellation must not rely on the cancelled context for its work. Each call site must reason about which context is alive at each execution point. Relevant files: `pkg/api/vm.go` — `vmExecuteCreate()` cleanup goroutine. |
| 103 | **Service vs Controller construction pattern** | **Service** constructors take repos/options only (no entity): `network.NewService(repo, tracker)`. **Controller** constructors take a specific entity + repos: `vm.NewController(ctx, vm, repo)`, `network.NewController(net, repo)`. The Go port renamed `LeaseService` to `LeaseController` because it follows the Controller construction pattern (per-entity binding to a `*model.Network`). This naming convention prevents confusion about whether a type belongs in `Operation.Services` (singleton, wired once at startup) or is created per-operation. **Rule:** If a type requires a specific entity to construct, it's a Controller, not a Service. |
...
│   │   │                             #   lease_controller, lease_repository, lease_sqlite,
