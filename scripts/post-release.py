...
def get_latest_tag(root: Path) -> str:
    ...
    try:
        ...
        if not validate_semver(version):
            print_fail(
                f"Latest tag '{tag}' does not contain a valid semver version."
            )
            sys.exit(1)
        return version
    except subprocess.CalledProcessError:
        print_fail(
            "No git tags found. Run bump-version.py first and create a tag."
        )
        sys.exit(1)
...
def regenerate_srcinfo(root: Path) -> None:
    ...
    try:
        ...
        if result.returncode != 0:
            print_warn(".SRCINFO: makepkg not found, skipping regeneration")
            print_info(
                "Install pacman-contrib or run manually: makepkg --printsrcinfo > .SRCINFO"
            )
            return
        ...
        print_success("packaging/.SRCINFO: regenerated")
    except subprocess.CalledProcessError as e:
        print_warn(".SRCINFO: failed to regenerate")
        print_info(f"{e.stderr}")
...
def update_aur(version: str, root: Path, dry_run: bool) -> None:
    ...
    binary_url = (
        f"https://github.com/AlanD20/mvmctl/releases/download/v{version}/mvm"
    )
    manpage_url = f"https://raw.githubusercontent.com/AlanD20/mvmctl/v{version}/docs/mvm.1"

    print_banner(f"Updating AUR PKGBUILD for v{version}")

    if dry_run:
        print_info("Would download:")
        print_info(f"{binary_url}")
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print_info("Downloading mvm binary...")
            ...
            try:
                ...
                print_success(f" mvm: {binary_hash[:16]}...")
            except Exception as e:
                print_fail(f"Failed to download binary: {e}")
                print_info(
                    f"Make sure GitHub release v{version} is published with the 'mvm' binary."
                )
                sys.exit(1)

            print_info("Downloading mvm.1 man page...")
            manpage_path = tmp_path / f"mvm.1-{version}"
            try:
                urllib.request.urlretrieve(manpage_url, manpage_path)
                manpage_hash = hashlib.sha256(
                    manpage_path.read_bytes()
                ).hexdigest()
                checksums.append(manpage_hash)
                print_success(f" mvm.1: {manpage_hash[:16]}...")
            except Exception as e:
                print_fail(f"Failed to download man page: {e}")
                print_info(
                    f"Make sure the docs/mvm.1 file exists in the v{version} tag."
                )
                sys.exit(1)

            content = pkgbuild_path.read_text()
            new_checksums_line = (
                f"sha256sums=('{checksums[0]}' '{checksums[1]}')"
            )
            new_content = re.sub(
                r"sha256sums=\([^)]+\)", new_checksums_line, content
            )
            pkgbuild_path.write_text(new_content)
            print_success("packaging/PKGBUILD: updated sha256sums")
        ...
        print_info(
            f"Make sure GitHub release v{version} is published with artifacts."
        )
        sys.exit(1)
...
def main() -> None:
  ...
  %(prog)s --aur --dry-run              # Preview changes
    ...
    parser.add_argument(
        "--aur",
        action="store_true",
        help="Update AUR PKGBUILD sha256sums and regenerate .SRCINFO",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.aur:
        ...
        parser.print_help()
        print_fail("No task specified. Use --aur to update AUR checksums.")
        sys.exit(1)
