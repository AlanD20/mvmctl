...
- ALL subprocess calls through `system.DefaultRunner.Run()` / `system.DefaultRunner.Stream()` with `system.RunCmdOpts` — no raw `os/exec`. Documented exceptions (see CONTEXT.md "Subprocess invocation"): Jailer exec (privileged chroot handoff with inherited console descriptors), SSH probe (connectivity detection), loopmount provisioner (chained losetup/mount/umount), vsock agent exec/pty (su/sh command execution), archive xz decompression (pipe-based), and the runner/spawn abstraction boundary itself (`runner.go`, `interactive_run.go`, `spawn.go`).
