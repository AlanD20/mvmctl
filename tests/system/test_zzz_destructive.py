...
class TestCacheCleanActual:
    ...
    def test_cache_clean_actual(self, mvm_binary) -> None:
        ...
        if result.returncode != 0:
            pytest.skip(
                f"cache clean --force failed (may need sudo): "
                f"{result.stderr}"
            )
        ...
        _run_mvm(
            mvm_binary, "init", "--non-interactive", "--skip-host",
            check=False,
        )
        ...
        _run_mvm(mvm_binary, "network", "create", "net",
                 "--subnet", "10.200.0.0/24", "--no-nat", check=False)
