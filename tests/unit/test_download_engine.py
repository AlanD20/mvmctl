...
class TestDownloadEngineInit:
    ...
    def test_creates_temp_dir_if_missing(self, tmp_path):
        """Engine creates temp directory during download if it doesn't exist."""
        new_dir = tmp_path / "new_temp"
        engine = DownloadEngine(temp_dir=new_dir)
        # Directory is created during _download_once, not in __init__
        assert engine.temp_dir == new_dir
        assert not new_dir.exists()  # Not created until download is called
