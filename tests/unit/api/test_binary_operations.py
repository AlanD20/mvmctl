...
from mvmctl.models.result import BatchResult
...
class TestBinaryPull:
    """Tests for BinaryOperation.pull()."""

    def test_pull_downloads_when_not_exists(self, mocker):
        """pull() downloads when version doesn't exist in DB."""
        ...
        mocker.patch(
            "mvmctl.api.binary_operations.BinaryRepository",
        ...
        mocker.patch(
            "mvmctl.api.binary_operations.BinaryPullRequest",
    ...
    def test_pull_early_return_when_exists(self, mocker):
        ...
        mock_repo.get_by_name_and_version.side_effect = lambda n, v: (
            fc if n == "firecracker" else jl
        )
        mocker.patch(
            "mvmctl.api.binary_operations.BinaryRepository",
            return_value=mock_repo,
    ...
    def test_pull_sets_default_when_no_default_exists(self, mocker):
        """pull() sets binary as default when no default exists."""
        mocker.patch("mvmctl.api.binary_operations.Database")
        mock_repo = MagicMock()
        mock_repo.get_by_name_and_version.return_value = None
...
class TestBinaryRemove:
    def test_remove_delegates(self, mocker):
        ...
        mocker.patch(
            "mvmctl.api.binary_operations.BinaryRequest",
            return_value=mock_request,
        )
    ...
    def test_remove_force(self, mocker):
        ...
        mocker.patch("mvmctl.api.binary_operations.BinaryRepository")
        mock_request = MagicMock()
        mock_request.resolve.return_value = MagicMock(binaries=[_make_bin()])
        mocker.patch(
            "mvmctl.api.binary_operations.BinaryRequest",
            return_value=mock_request,
        )
        ...
        result = BinaryOperation.remove(
            MagicMock(identifiers=["bin-001"]), force=True
        )
        assert isinstance(result, BatchResult)
...
class TestBinaryRemoveByVersion:
    def test_removes_both_binaries(self, mocker):
        ...
        mocker.patch(
            "mvmctl.api.binary_operations.BinaryResolver",
...
class TestBinaryListRemote:
    ...
    def test_list_remote_default_limit(self, mocker):
        mocker.patch("mvmctl.api.binary_operations.Database")
        mocker.patch(
            "mvmctl.api.binary_operations.SettingsService.resolve",
            return_value="10",
        )
        mocker.patch(
            "mvmctl.api.binary_operations.BinaryService.list_remote",
