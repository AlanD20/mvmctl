...
class TestSudoCredentials:
    """Tests for _is_sudo_cached() and _validate_sudo_credentials()."""

    def test_is_sudo_cached_not_valid(self, monkeypatch):
        import mvmctl.utils._system as _sys
        from mvmctl.utils._system import _is_sudo_cached

        monkeypatch.setattr(_sys, "_SUDO_CREDENTIALS_VALID", False)
        assert _is_sudo_cached() is False

    def test_is_sudo_cached_expired(self, monkeypatch):
        import mvmctl.utils._system as _sys
        from mvmctl.utils._system import _is_sudo_cached

        monkeypatch.setattr(_sys, "_SUDO_CREDENTIALS_VALID", True)
        monkeypatch.setattr(_sys, "_SUDO_CACHE_TIMESTAMP", 0.0)
        assert _is_sudo_cached() is False

    def test_is_sudo_cached_valid(self, monkeypatch):
        import mvmctl.utils._system as _sys
        from mvmctl.utils._system import _is_sudo_cached

        monkeypatch.setattr(_sys, "_SUDO_CREDENTIALS_VALID", True)
        monkeypatch.setattr(_sys, "_SUDO_CACHE_TIMESTAMP", 1e10)
        assert _is_sudo_cached() is True

    def test_validate_anti_recursion(self, monkeypatch):
        import mvmctl.utils._system as _sys
        from mvmctl.utils._system import _validate_sudo_credentials

        monkeypatch.setattr(_sys, "_SUDO_VALIDATION_IN_PROGRESS", True)
        assert _validate_sudo_credentials() is False

    def test_validate_cached_path(self):
        from mvmctl.utils._system import _validate_sudo_credentials

        assert _validate_sudo_credentials() is True

    def test_validate_sudo_n_succeeds(self, mocker, monkeypatch):
        import mvmctl.utils._system as _sys
        from mvmctl.utils._system import _validate_sudo_credentials

        monkeypatch.setattr(_sys, "_SUDO_CREDENTIALS_VALID", False)
        mock_run = mocker.patch("mvmctl.utils._system.subprocess.run")
        mock_run.return_value = MagicMock(returncode=0)
        assert _validate_sudo_credentials() is True

    def test_validate_sudo_v_succeeds_after_n_fails(self, mocker, monkeypatch):
        import mvmctl.utils._system as _sys
        from mvmctl.utils._system import _validate_sudo_credentials
    ...
    def test_validate_both_fail(self, mocker, monkeypatch):
        import mvmctl.utils._system as _sys
        from mvmctl.utils._system import _validate_sudo_credentials
