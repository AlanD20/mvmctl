...
|---|-------------|--------|-------|
| 1 | New constants in `constants.py` | Done | PROJECT_GROUP, SUDOERS_DROP_IN_PATH, DEFAULT_NETWORK_*, BRIDGE_PREFIX, FIRECRACKER_*_TIMEOUT, PRIVILEGED_BINARIES; TAP_PREFIX changed to "fcm-tap" |
...
| 7 | `check_privileges` in `api/host.py` | Done | Checks binary exists + user is root or in fcm group |
...
| 13 | Top-level `fcm help` command | Done | Navigates Click command tree for subcommand help |
| 14 | `configure.py` updated for privilege model | Done | Step 1 explains sudo-once model, offers sudo fcm host init |
