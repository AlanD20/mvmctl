...
def test_create_network_setup_failure(mock_teardown_bridge, mock_setup_bridge, mock_cache_dir: Path):
...
@patch("fcm.core.network_manager.teardown_bridge")
@patch("fcm.core.network_manager.teardown_nat")
def test_remove_network_partial_failure(mock_teardown_nat, mock_teardown_bridge, mock_cache_dir: Path):
    net_dir = mock_cache_dir / "networks" / "mynet"
    net_dir.mkdir(parents=True)
    _save_config(net_dir, NetworkConfig("mynet", "10.20.1.0/24", "10.20.1.1", "fcm-mynet"))
    _save_leases(net_dir, [])
...
def test_ensure_default_network(mock_create_network, mock_cache_dir: Path):
    ...
    mock_create_network.return_value = NetworkConfig("default", "10.20.0.0/24", "10.20.0.1", "fcm-br0")
...
def test_auto_allocate_subnet_exhausted(mock_cache_dir: Path):
    ...
    for i in range(256):
        ...
        net_dir.mkdir(parents=True)
        _save_config(net_dir, NetworkConfig(f"net{i}", f"10.20.{i}.0/24", f"10.20.{i}.1", f"fcm-net{i}"))
