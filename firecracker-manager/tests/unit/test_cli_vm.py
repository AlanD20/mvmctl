...
def test_list_vms_json():
    vm = _make_vm("myvm")
    with patch("fcm.cli.vm.list_vms", return_value=[vm]):
...
def test_list_vms_all_flag():
    ...
    with patch("fcm.cli.vm.list_vms", return_value=[running_vm, stopped_vm]):
...
def test_delete_force_running_vm():
    ...
    with patch("fcm.cli.vm.get_vm", return_value=vm), patch("fcm.cli.vm.remove_vm"):
...
def test_cleanup_with_vms():
    stopped_vm = _make_vm("vm-stopped", VMState.STOPPED, "10.20.0.3")
    with patch("fcm.cli.vm.list_vms", return_value=[stopped_vm]), patch("fcm.cli.vm.cleanup_vms"):
