...
class VMExportConfig:
    ...
    **NEVER add:** image_id, kernel_id, binary_id, network_id — those are internal.
    ...
    compute: VMExportComputeConfig = field(
        default_factory=VMExportComputeConfig
    )
    ...
    network: VMExportNetworkConfig = field(
        default_factory=VMExportNetworkConfig
    )
    boot: VMExportBootConfig = field(default_factory=VMExportBootConfig)
    firecracker: VMExportFirecrackerConfig = field(
        default_factory=VMExportFirecrackerConfig
    )
    cloud_init: VMExportCloudInitConfig = field(
        default_factory=VMExportCloudInitConfig
    )
    ...
    def from_dict(cls, data: dict[str, Any]) -> VMExportConfig:
        ...
        for field_name, field_class in [
            ...
            if field_name in data and isinstance(data[field_name], dict):
                ...
                sub_filtered = {
                    k: v for k, v in sub_data.items() if k in sub_known
                }
        ...
        filtered = {
            ...
            if k in known_fields and k not in sub_configs
        }
    ...
    def from_json_file(cls, path: Path) -> VMExportConfig:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid JSON in VM config file {path}: {e}"
            ) from e
