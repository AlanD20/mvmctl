...
class KeyManager:
    ...
    @staticmethod
    def get_pubkey(key_name: str, db: MVMDatabase | None = None) -> str:
        """Get the public key content for a key by name."""
        db = db if db is not None else MVMDatabase()

        ssh_key = db.get_ssh_key_by_name(key_name)
        if ssh_key is None:
    ...
    def get_pubkeys(key_names: list[str], db: MVMDatabase | None = None) -> list[str]:
        ...
        for name in key_names:
            contents.append(KeyManager.get_pubkey(name, db))
        return contents
    ...
    def add_key(self, name: str, pub_key_path: str | Path, overwrite: bool = False) -> SSHKey:
        ...
        existing = self._db.get_ssh_key_by_name(name)
        if existing is not None:
            if overwrite:
                old_pub = self._get_keys_config_dir() / f"{name}.pub"
                if old_pub.exists():
                    old_pub.unlink()
                self._db.delete_ssh_key(existing.id)
            else:
        ...
        self._db.upsert_ssh_key(ssh_key)
    ...
    def create_key(
        ...
        existing_key = self._db.get_ssh_key_by_name(name)
        if existing_key is not None:
            if overwrite:
                self._db.delete_ssh_key(existing_key.id)
            else:
    ...
    def remove(self) -> None:
        ...
        self._db.delete_ssh_key(self._key.id)
    ...
    @staticmethod
    def set_default_keys(names: list[str], db: MVMDatabase | None = None) -> None:
        """Set the default SSH keys list used when creating VMs without --ssh-key."""
        db = db if db is not None else MVMDatabase()

        # Bulk lookup - get all keys first
        all_keys = db.list_ssh_keys()
        ...
        # Use bulk update in single SQL transaction
        key_ids = [name_to_key[name].id for name in names if name in name_to_key]
        db.set_default_ssh_keys_bulk(key_ids)

        logger.info("Set default SSH keys: %s", names)

    @staticmethod
    def get_default_keys(db: MVMDatabase | None = None) -> list[str]:
        """Get the list of default SSH key names."""
        db = db if db is not None else MVMDatabase()
        default_keys = db.get_default_ssh_keys()
        return [key.name for key in default_keys]

    @staticmethod
    def clear_default_keys(db: MVMDatabase | None = None) -> None:
        """Clear all default SSH keys."""
        db = db if db is not None else MVMDatabase()
        db.clear_default_ssh_keys()
        logger.info("Cleared default SSH keys")

    @staticmethod
    def list_keys(db: MVMDatabase | None = None) -> list[SSHKey]:
        """List all keys in the cache."""
        db = db if db is not None else MVMDatabase()
        return db.list_ssh_keys()
